    ...
    for (i=0; i<NUM_FRAMES; i++){
       ...
       //arm_rfft_q15(&instance, s+i*FFT_WINDOW, temp_data.raw_fft);
        ...
        for (j = 0; j < FFT_WINDOW; j++){
            temp_data.raw_fft[2*j] =  s[j+FFT_WINDOW*i];
        }

        fix_fftr(temp_data.raw_fft, 8, 0);
       ...
       for(j = 0; j < FFT_WINDOW * 2; j += 2) {
           ...
           temp_data.real[ j/2 ] = sqrtf((temp_data.raw_fft[j]     * temp_data.raw_fft[j]) +
                                               (temp_data.raw_fft[j + 1] * temp_data.raw_fft[j + 1]));
       }
